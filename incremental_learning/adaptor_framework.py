...
class LabelAdapter(nn.Module):
    def __init__(self, x_dim, edges, num_head=4, temperature=4, hid_dim=32):
        super().__init__()
        self.num_head = num_head
        self.linear = GCNConv(x_dim, hid_dim)
...
class ForecastModel(nn.Module):
    def __init__(self, model: nn.Module, x_dim: int = None, lr: float = 0.001, weight_decay: float = 0,
        ...
        Args:
            model (nn.Module): the forecast model - gnn model
...
class DoubleAdapt(ForecastModel):
    """
        bring all these stuff to the StockNet
    """
    def __init__(
